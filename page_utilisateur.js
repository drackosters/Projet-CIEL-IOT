document.addEventListener("DOMContentLoaded", function () {
  ...
  // Fermer le panneau d'ajout IoT si on clique en dehors
  document.addEventListener('click', function (event) {
      const conteneurAjout = document.getElementById('conteneur-ajout-iot');
      const boutonAjout = document.querySelector('.bouton-ajout-iot');
  
      if (!conteneurAjout || !boutonAjout) return;
  
      const isClickInside = conteneurAjout.contains(event.target) || boutonAjout.contains(event.target);
  
      if (!isClickInside) {
          conteneurAjout.style.display = 'none';
      }
  });
